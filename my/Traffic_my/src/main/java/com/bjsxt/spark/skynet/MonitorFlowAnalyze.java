...
public class MonitorFlowAnalyze {
	public static void main(String[] args) {
		// 构建Spark运行时的环境参数
		SparkConf conf = new SparkConf().setAppName(Constants.SPARK_APP_NAME_SESSION)// .set("spark.sql.shuffle.partitions",
																						// "10")
		...
		/**
		 * 基于本地测试生成模拟测试数据，如果在集群中运行的话，直接操作Hive中的表就可以 本地模拟数据注册成一张临时表 monitor_flow_action
		 * monitor_camera_info
		 */
		...
		/*
		 * 检测卡口状态 carCount2MonitorRDD K:car_count V:monitor_id
		 */
		...
		/*
		 * 获取车流量排名前N的卡口号 return KV格式的RDD K：monitor_id V:monitor_id
		 */
		JavaPairRDD<String, String> topNMonitor2CarFlow = getTopNMonitorCarFlow(sc, taskId, taskParamsJsonObject,
				carCount2MonitorRDD);
...
//		getMonitorDetails(sc,taskId,top5MonitorIds,monitor2DetailRDD);   
	...
	private static void getTopNDetails(final long taskId, JavaPairRDD<String, String> topNMonitor2CarFlow,
			JavaPairRDD<String, Row> monitor2DetailRDD) {
		/*
		 * 获取车流量排名前N的卡口的详细信息 可以看一下是在什么时间段内卡口流量暴增的。
		 */
		/*
		 * 优化点： 因为topNMonitor2CarFlow 里面有只有5条数据，将这五条数据封装到广播变量中，然后遍历monitor2DetailRDD
		 * ，每遍历一条数据与广播变量中的值作比对。
		 */
	 ...
	 * @param carCount2MonitorRDD
	 * @return
	 */
	private static JavaPairRDD<String, String> getTopNMonitorCarFlow(JavaSparkContext sc, long taskId,
			JSONObject taskParamsJsonObject, JavaPairRDD<Integer, String> carCount2MonitorId) {
		/*
		...
		/*
		 * carCount2Monitor<carCount, monitor_id>
		 */
	...
	private static JavaPairRDD<Integer, String> checkMonitorState(JavaSparkContext sc, SQLContext sqlContext,
			JavaPairRDD<String, String> monitorId2CameraCountRDD, long taskId, JSONObject taskParamsJsonObject,
		...
		/*
		 * 对每一个卡口下面的信息进行统计，统计出来camera_count（这个卡口下一共有多少个摄像头），camera_ids（这个卡口下，
		 * 所有的摄像头编号拼接成的字符串） 如何统计？ 1、按照monitor_id分组 2、使用mapToPair遍历，遍历的过程可以统计
		 */
					...
					public Iterable<Tuple2<Integer, String>> call(
						...
						while (iterator.hasNext()) {
							...
							if (factCameraCount == standardCameraCount) {
								/*
								 * 1、正常卡口数量 2、异常卡口数量 3、正常通道（此通道的摄像头运行正常）数 4、异常卡口数量中哪些摄像头异常，需要保存摄像头的编号
								 */
	...
	private static JavaPairRDD<String, String> aggreagteByMonitor(
			JavaPairRDD<String, Iterable<Row>> monitorId2RowsRDD) {
		/*
		 * <monitor_id,List<Row> 集合里面的记录代表的是camera的信息。>
		 */

		/*
		 * 一个monitor_id对应一条记录 为什么使用mapToPair来遍历数据，因为我们要操作的返回值是每一个monitorid 所对应的详细信息
		 */
		JavaPairRDD<String, String> monitorId2CameraCountRDD = monitorId2RowsRDD
